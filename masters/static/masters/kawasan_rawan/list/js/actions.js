async function handleDetail(id, tahun) {
    ...
    let wilayah = data.wilayah;

    const { nama_desa, nama_kecamatan, nama_kabupaten, nama_provinsi } = wilayah;

    let waktu = moment(data.created_at).format('HH:mm:ss - DD MMMM YYYY');
    let keterangan = data.keterangan == null ? '-' : data.keterangan;
    ...
    if (created_by) {
        let pelapor_nama = created_by.first_name + ' ' + created_by.last_name;
        let pelapor_username = created_by.username;
        let pelapor_email = created_by.email;
        let pelapor_notelp = created_by.profile.notelp;

        detail_pelapor.find('.detail_pelapor_nama').text(pelapor_nama);
        detail_pelapor.find('.detail_pelapor_username').text(pelapor_username);
        detail_pelapor.find('.detail_pelapor_email').text(pelapor_email);
        detail_pelapor.find('.detail_pelapor_notelp').text(pelapor_notelp);
    }
    ...
    // modal.find('.detail_edit').attr('href', `/dashboard/masters/kawasan_rawan/edit/${data_kawasan.id}/`);
    // modal.find('.detail_edit').attr('onclick', `handleEdit(${id}, ${tahun})`);
