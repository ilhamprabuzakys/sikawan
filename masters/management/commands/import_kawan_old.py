...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        index_status = options['index_status']
        failed_file = None
        ...
        try:
            ...
            with open(file_path, 'r', encoding='utf-8-sig') as csv_file:
                ...
                for row in csv_reader:
                    line_count += 1
                    try:
                        kab, kec, desa, status = row[2].strip(), row[4].strip(), row[6].strip(), row[index_status].strip().upper()
                        ...
                        if dbdesa:
                            ...
                            KawasanRawan.objects.create(desa=dbdesa, tahun=tahun, status=status)
                            processed_count += 1
